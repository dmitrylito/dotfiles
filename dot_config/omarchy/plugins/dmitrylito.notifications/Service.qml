...
Item {
  ...
  function handleNotification(notification) {
    ...
    notification.closed.connect(function(reason) {
      var mine = service.liveRefs[snapshot.originalId] === notification
      if (mine) delete service.liveRefs[snapshot.originalId]
      // A sender closing its own notification (Dialpad clearing an answered
      // call) must take the toast with it: a critical popup never expires,
      // so without this it stays on screen forever.
      if (mine && reason === NotificationCloseReason.CloseRequested)
        service.closePopupBySender(snapshot.originalId, snapshot.timestamp)
    })
  ...
  // The sender asked the server to close this notification. Archive the row
  // like any other departing toast; liveRefs is already cleared, so
  // removePopup makes no dismiss call back at a torn-down object.
  function closePopupBySender(originalId, timestamp) {
    for (var i = popupModel.count - 1; i >= 0; i--) {
      var row = popupModel.get(i)
      if (!row || row.originalId !== originalId || row.timestamp !== timestamp) continue
      if (isRestoredRow(row)) continue
